...
const store = derived<typeof theme, Colors>(theme, ($theme) =>
	$theme == 'light' ? lighColors : darkColors
);
...
store.subscribe((newColors) => {
	if (browser) {
		...
		document.documentElement.style.setProperty(
			'--global-light-contrast-color',
			newColors.lightContrast
		);
		document.documentElement.style.setProperty(
			'--global-thin-contrast-color',
			newColors.thinContrast
		);
